export function GetDateTimeFromTags(date: string, time?: string): Date {
    if (!date) {
        return null;
    }
...
export function getDwordFromBuffer(buffer: Uint8Array, index: number) {
    return buffer[index] | (buffer[index + 1] << 8) | (buffer[index + 2] << 16) | (buffer[index + 3] << 24);
}

export function padLeft(str: string, num: number) {
...
export function MakeTag(group: number, elem: number): string {
